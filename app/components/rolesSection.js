    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6">
        {roles.map((role, index) => (
          <div key={index} className="bg-white bg-opacity-10 p-4 rounded-lg backdrop-filter backdrop-blur-lg transition-all duration-300 hover:bg-opacity-20 hover:transform hover:scale-105">
            <div className="flex items-center mb-2">
              <role.icon className="text-3xl text-yellow-300 mr-3" />
              <h3 className="text-xl font-bold text-white">{role.title}</h3>
            </div>
            <p className="text-sm text-white">{role.description}</p>
          </div>
        ))}
      </div>
    </div>
